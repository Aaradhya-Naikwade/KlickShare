...
interface GroupItem {
  ...
  photos?: string[];
  members?: any[];
  event?: EventInfo; // ✅ IMPORTANT
}

type GroupedByEvent = Record<
  string,
  {
    eventName: string;
    groups: GroupItem[];
...
export default function JoinedGroups() {
  ...
  // GROUP BY EVENT
  // --------------------------------------------------
  const grouped: GroupedByEvent = groups.reduce((acc, g) => {
    const eventId = g.event?._id || "unknown";
    const eventName = g.event?.name || "Other Events";
    ...
    acc[eventId].groups.push(g);
    return acc;
  }, {} as GroupedByEvent);

  if (loading)
    return (
      <div className="p-6 text-[#1f6563]">
  ...
  return (
    <div className="p-6 space-y-10">
      ...
      {Object.keys(grouped).length === 0 ? (
        ...
        Object.entries(grouped).map(([eventId, block]) => (
          <div key={eventId} className="space-y-4">
            {/* EVENT TITLE */}
            <h3 className="text-xl font-semibold text-[#15514f] border-l-4 border-[#1f6563] pl-3">
              {block.eventName}
            </h3>
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-3 gap-3">
                    {/* <Link
                      ...
                      className="text-sm bg-[#1f6563] text-white px-3 py-2 rounded hover:bg-[#15514f]"
                    >
                      Open
                    </Link> */}
